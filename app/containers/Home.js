...
import { StyleSheet, View, Text, Image, DrawerLayoutAndroid, ScrollView, RefreshControl } from 'react-native'
  ...
  onItemClick = (item) => {
    if(!item.url){
      ...
      this.props.dispatch(createAction('book/searchBookUrl')({
        ...
        onSuccess: () => {
          Toast.hide();
          this.props.dispatch(NavigationActions.navigate({routeName: 'Read', params:{id:item.id}}));
        },
        onError: () => {
    ...
    return (
      ...
      >
        ...
        <View style={styles.list}>
          {bookList.map( (item,i) => (
            <Touchable key={i} style={styles.item} onPress={()=>this.onItemClick(item)} onLongPress={()=>this.onLongClick(item)}>
              <CachedImage
                source={{uri: item.img}}
              ...
              <Text style={styles.title}>{item.progress ? item.progress : '0.00%'}</Text>
              {item.hasNew? (

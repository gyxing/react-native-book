...
export default class extends Component {
  ...
  componentDidMount() {
    this.props.dispatch(createAction('book/getBook')({
      id: this.bookId,
      callback: (book) => {
        this.props.dispatch(createAction('book/getChapters')({
          bookId: this.bookId,
          callback: (chapterList) => {
            let chapter = chapterList[0];
            if(book.curChapterId) { chapter = chapterList.find( item => item.id===book.curChapterId) }
    ...
    Storage.get('readSetup', {fontSize:18, paper:'paper', night:false}).then( setup => {
      this.setState({...setup})
    });
  }
  ...
  setChapter = (chapter, book, chapterList) => {
    ...
    if(!chapterList){ chapterList = this.state.chapterList }

    if(chapter.content){
      this.setState({ book, chapter, chapterList, modalVisible:false, popupMode:'' });
      this.scrollToTop();
      ...
      this.props.dispatch(createAction('book/searchContent')({
        chapter,
        callback: (content) => {
          Toast.hide();
    ...
    if(book.curChapterId !== chapter.id){
      this.props.dispatch(createAction('book/updateBook')({
        book,
        params: {
          curChapterId: chapter.id,
          progress: `${((chapterList.findIndex(item=>item.id===chapter.id)+1)/chapterList.length*100).toFixed(2)}%`
        }
      }))
    }
  };
  ...
  onPrevious = () => {
    let {chapter, chapterList} = this.state;
    let preChapter = chapterList.find( item => item.id===chapter.id-1 );
    if(preChapter){
      this.setChapter(preChapter)
    }
  };

  onNext = () => {
    let {chapter, chapterList} = this.state;
    let nextChapter = chapterList.find( item => item.id===chapter.id+1 );
    if(nextChapter){
      this.setChapter(nextChapter)
    }
  };
  ...
  /* FlatList */
  scrollTo = (index) => {
    this._listView.scrollToIndex({animated: true, index: index});
  };
  ...
  onCache = (len) => {
    ...
    let {chapter, chapterList} = this.state;
    let index = chapterList.findIndex( item => item.id === chapter.id );
    if(len === -1){
  ...
  _cacheChapter = (index, len, count) => {
    ...
    if(count <= len){
      let chp = chapterList[index+count];
      if(chp && !chp.content){
        this.props.dispatch(createAction('book/searchContent')({
          chapter: chp,
          callback: (content) => {
  ...
  renderContent = () => {
    const {chapter, fontSize, paper, night} = this.state;
    let imgSource = null, color = '#333';
    if(night){
    ...
    return (
      <ImageBackground source={imgSource} style={{width:appWidth,height:'100%'}}>
        {!chapter.content? (
          <Loading/>
        ) : (
          <ScrollView style={{flex:1}} ref={ref=>this._scrollView=ref}>
            <Touchable activeOpacity={1} style={{flex:1}}  onPress={()=>this.setState({modalVisible:true})}>
              <Text style={[styles.name, {fontSize:fontSize+2, color}]}>{chapter.name}</Text>
              <Text style={[styles.content, {fontSize,color}]}>{chapter.content}</Text>
            </Touchable>
            <Touchable style={{justifyContent:'center',alignItems:'center',height:60, borderTopColor:'#eee',borderTopWidth:0.5}} onPress={this.onNext}>
  ...
  renderModal = () => {
    ...
    let color = '#082646';
    return modalVisible? (
      <Touchable activeOpacity={1} style={styles.modal} onPress={()=>this.setState({modalVisible:false, popupMode:''})}>
        <View style={styles.head}>
          <Text numberOfLines={1} style={{color:'#333'}}>{book.name}　{chapter.name}</Text>
        </View>
        <View style={styles.center}>
          <Touchable style={styles.left} onPress={()=>this.drawer.openDrawer()}>
            <Icon type={AppFont.doubleRight} color="#666" />
          </Touchable>
          <Touchable style={styles.right}>
            <Text style={[styles.rightI,{borderBottomWidth:0.5,borderBottomColor:'#eee',color}]} onPress={this.onPrevious}>上一章</Text>
            <Text style={[styles.rightI,{color}]} onPress={this.onNext}>下一章</Text>
          </Touchable>
        </View>
        <View style={styles.foot}>
          <Touchable style={styles.footI} onPress={()=>this.props.dispatch(NavigationActions.back())}>
            <Icon type={AppFont.left} size={23} color={color} />
          </Touchable>
          <Touchable style={styles.footI} onPress={()=>this.setState({popupMode:popupMode==='font'?'':'font'})}>
            <Icon type={AppFont.font} size={23} color={color} />
          </Touchable>
          <Touchable style={styles.footI} onPress={()=>this.setState({popupMode:popupMode==='paper'?'':'paper'})}>
            <Icon type={AppFont.light} size={28} color={color} />
          </Touchable>
          <Touchable style={styles.footI} onPress={()=>{this.setState({popupMode:''});this.onSetup('night',!night)}}>
            <Icon type={AppFont.moon} size={20} color={color} />
          </Touchable>
          <Touchable style={styles.footI} onPress={()=>this.setState({popupMode:popupMode==='more'?'':'more'})}>
      ...
      <View style={{flex:1,backgroundColor:'#efefef',paddingHorizontal:20}}>
        <View style={{height:50,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
          ...
          <View style={{flexDirection:'row'}}>
            ...
            <Touchable style={[styles.tag,{borderRightWidth:0}]} onPress={()=>this.scrollTo(chapterList.length-1)}>
    ...
    if(popupMode === 'font'){
      return (
        <View style={[styles.popup, {width:240, left:appWidth/2-120}]}>
          <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'flex-start', height:30, paddingHorizontal:8}}>
            <Text>字体：{fontSize}</Text>
            <Button title="" text="默认" textStyle={{fontSize:10}} style={{paddingHorizontal:5, paddingVertical:3}} onPress={()=>this.onSetup('fontSize',18)} />
          </View>
          <View style={{flex:1}}>
            <Slider defaultValue={fontSize} min={10} max={40} step={2} onAfterChange={val=>this.onSetup('fontSize',val)}/>
          </View>
        </View>
      )
    }else if(popupMode === 'paper'){
      return (
        <View style={[styles.popup, {width:180, left:appWidth/2-90, flexDirection:'row', justifyContent:'space-between', paddingHorizontal:20}]}>
          <Touchable onPress={()=>this.onSetup('paper','default')}>
            <Image source={require('../images/pages/default.png')} style={styles.paper} />
          </Touchable>
          <Touchable onPress={()=>this.onSetup('paper','paper')}>
            <Image source={require('../images/pages/paper.jpg')} style={styles.paper} />
          </Touchable>
        </View>
      )
      ...
      return (
        <View style={[styles.popup, {width:200, left:appWidth/2-100}]}>
          ...
          <Touchable style={[styles.line, {borderBottomWidth:0}]} onPress={this.onGoSwitching}>
